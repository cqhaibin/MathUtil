...
namespace MathUtil.Geometry
{
    ...
    public readonly struct Polygon : IPolygon
    {
        ...
        {
            return MathUtil.Algorithm.GeometryAlgorithm.IsConvexPolygon(_vertexes);
        }
        ...
        public static bool ValidityCheck(params Point2D[] vertexes)
        {
            ...
            for (int i = 0, j, k; i < vertexes.Length; i++)
            {
                ...
                if (MathUtil.Algorithm.GeometryAlgorithm.IsCollinearity(vertexes[i], vertexes[j], vertexes[k]))
                    return false;
            }
            if (MathUtil.Algorithm.GeometryAlgorithm.IsSelfIntersected(vertexes))
                return false;
            return true;
        }
        #endregion
